	    ...
	    _createClass(CheckList, [{
	        ...
	        value: function _build(colIndex) {
	            ...
	            for (var k = tf.refRow; k < tf.nbRows; k++) {
	                ...
	                for (var j = 0; j < ncells; j++) {
	                    if (colIndex === j && (!tf.refreshFilters || tf.refreshFilters && tf.disableExcludedOptions) || colIndex === j && tf.refreshFilters && (rows[k].style.display === '' && !tf.paging || tf.paging && (!activeFlt || activeFlt === colIndex || activeFlt != colIndex && _Arr2['default'].has(tf.validRowsIndex, k)))) {
	        ...
	        value: function addChecks(colIndex, ul) {
	            ...
	            for (var y = 0; y < this.opts.length; y++) {
	                ...
	                if (tf.refreshFilters && tf.disableExcludedOptions && _Arr2['default'].has(excludedOpts, _Str2['default'].matchCase(val, tf.matchCase), tf.matchCase)) {
