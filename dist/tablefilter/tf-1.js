	            ...
	            for (var ucol = 0; ucol <= ucolMax; ucol++) {
	                ...
	                for (var i = 0; i <= mThisCol; i++) {
	                    ...
	                    if (oTypeThisCol && result) {
	                        ...
	                        if ((0, _dom.byId)(labThisCol[i])) {
	                            switch (oTypeThisCol.toLowerCase()) {
	                                case 'innerhtml':
	                                    if (isNaN(result) || !isFinite(result) || nbvalues === 0) {
	                                        (0, _dom.byId)(labThisCol[i]).innerHTML = '.';
	                                    } else {
	                                        (0, _dom.byId)(labThisCol[i]).innerHTML = result;
	                                    }
	                                    break;
	                                case 'setvalue':
	                                    (0, _dom.byId)(labThisCol[i]).value = result;
	                                    break;
	                                case 'createtextnode':
	                                    var oldnode = (0, _dom.byId)(labThisCol[i]).firstChild;
	                                    var txtnode = (0, _dom.createText)(result);
	                                    (0, _dom.byId)(labThisCol[i]).replaceChild(txtnode, oldnode);
	                                    break;
	                            } //switch
	                        }
	                    } else {
	                            try {
	                                if (isNaN(result) || !isFinite(result) || nbvalues === 0) {
	                                    ...
	                                    (0, _dom.byId)(labThisCol[i]).innerHTML = result.toFixed(precision);
	                                }
	                            } catch (e) {} //catch
	                        } //else
	                } //for i
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ColsVisibility = function (_Feature) {
	    ...
	    function ColsVisibility(tf, f) {
	        ...
	        _this.hiddenCols = [];
	        _this.tblHasColTag = (0, _dom.tag)(tf.tbl, 'col').length > 0;
	    ...
	    ColsVisibility.prototype.buildBtn = function buildBtn() {
	        ...
	        var targetEl = !this.btnTgtId ? tf.rDiv : (0, _dom.byId)(this.btnTgtId);
	    ...
	    ColsVisibility.prototype.buildManager = function buildManager() {
	        ...
	        var container = !this.contElTgtId ? (0, _dom.createElm)('div', ['id', this.prfxCont + tf.id]) : (0, _dom.byId)(this.contElTgtId);
	        ...
	        if (this.enableTickAll) {
	            (function () {
	                var li = createCheckItem('col__' + tf.id, _this4.tickAllText, _this4.tickAllText);
	                ...
	                _event2.default.add(li.check, 'click', function () {
	                    for (var h = 0; h < headerRow.cells.length; h++) {
	                        var itm = (0, _dom.byId)('col_' + h + '_' + tf.id);
	        ...
	        for (var i = 0; i < headerRow.cells.length; i++) {
	            ...
	            var liElm = createCheckItem('col_' + i + '_' + tf.id, cellText, cellText);
	    ...
	    ColsVisibility.prototype.showCol = function showCol(colIndex) {
	        ...
	        if (this.manager && this.contEl) {
	            var itm = (0, _dom.byId)('col_' + colIndex + '_' + this.tf.id);
	            if (itm) {
	                itm.click();
	            }
	        } else {
	    ...
	    ColsVisibility.prototype.destroy = function destroy() {
	        ...
	        if ((0, _dom.byId)(this.contElTgtId)) {
	            (0, _dom.byId)(this.contElTgtId).innerHTML = '';
	        } else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var FiltersVisibility = function (_Feature) {
	    ...
	    FiltersVisibility.prototype.buildUI = function buildUI() {
	        ...
	        var targetEl = !this.targetId ? tf.rDiv : (0, _dom.byId)(this.targetId);
	        ...
	        SortableTable.prototype.updateHeaderArrows = function () {
	            ...
	            if (adpt.asyncSort && adpt.triggerIds.length > 0) {
	                ...
	                for (var j = 0; j < triggers.length; j++) {
	                    cells.push((0, _dom.byId)(triggers[j]));
	                }
	            } else {
	                if (!this.tHead) {
	                    return;
	        ...
	        if (this.asyncSort && this.triggerIds.length > 0) {
	            (function () {
	                ...
	                for (var j = 0; j < triggers.length; j++) {
	                    ...
	                    var trigger = (0, _dom.byId)(triggers[j]);
	                    if (trigger) {
